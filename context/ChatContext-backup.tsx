...
export const ChatProvider: React.FC<ChatProviderProps> = ({ children }) => {
  ...
  const loadConversation = useCallback(async (conversationId: string) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Join WebSocket conversation for real-time updates (if connected)
  ...
  }, [user, isWebSocketConnected, markAsRead]);
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      ...
      if (success) {
        // Always reload messages to ensure they appear
        // WebSocket will handle real-time updates if connected
        setTimeout(() => {
  ...
  useEffect(() => {
    if (user && user.token) {
      // Always start with polling as fallback
      startPolling();
      
      // Try to initialize WebSocket connection
      chatService.initializeWebSocket(user.token).then(connected => {
        setIsWebSocketConnected(connected);
        if (connected) {
          console.log('✅ WebSocket connected, stopping polling');
          stopPolling();
        } else {
          console.log('❌ WebSocket failed, keeping polling fallback');
  ...
  useEffect(() => {
    ...
    const handleTyping = (data: any) => {
      // Handle typing indicators - you can implement this in your UI
    ...
    const handleConnection = (data: any) => {
      setIsWebSocketConnected(data.success);
    };
