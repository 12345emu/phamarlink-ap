...
export default function DoctorEditProfileModal({ 
  ...
  useEffect(() => {
    if (visible && user) {
      ...
      // Don't set profileImageUri from profileImage - let getProfileImageUrl handle it
      // profileImageUri should only be set when user selects a new image
  ...
  const handleSave = async () => {
    ...
    console.log('🔍 EditProfileModal - profileImageUri:', profileImageUri);
    console.log('🔍 EditProfileModal - profileImage:', profileImage);
    ...
    setLoading(true);
    try {
      console.log('🔍 EditProfileModal - About to call updateUserProfile...');
      ...
      console.log('✅ EditProfileModal - Profile data updated');
  ...
  const getProfileImageUrl = () => {
    console.log('🔍 EditProfileModal - getProfileImageUrl called');
    console.log('🔍 EditProfileModal - profileImageUri:', profileImageUri);
    console.log('🔍 EditProfileModal - profileImage:', profileImage);
    console.log('🔍 EditProfileModal - imageLoadError:', imageLoadError);
    
    // If there was an image load error, don't try to load the image again
    if (imageLoadError) {
      console.log('🔍 EditProfileModal - Image load error detected, returning null');
      return null;
    }
    
    // Priority 1: Use profileImageUri if it's a valid local file path (for new selections)
    if (profileImageUri && !profileImageUri.startsWith('http')) {
      console.log('🔍 EditProfileModal - Using local profileImageUri:', profileImageUri);
      return profileImageUri;
    }
    
    // Priority 2: Use profileImageUri if it's a valid network URL (for new selections)
    if (profileImageUri && profileImageUri.startsWith('http')) {
      console.log('🔍 EditProfileModal - Using network profileImageUri:', profileImageUri);
      return profileImageUri;
    }
    ...
    console.log('🔍 EditProfileModal - No valid image available, returning null');
    return null;
  };
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.profileImageSection}>
            ...
            {(() => {
              ...
              console.log('🔍 EditProfileModal - Rendering image with URL:', imageUrl);
              ...
              if (imageUrl) {
                return (
                  <Image
                    ...
                    onError={(error) => {
                      console.warn('❌ EditProfileModal - Failed to load profile image:', error);
                      console.warn('❌ EditProfileModal - Image URL was:', imageUrl);
                      setImageLoadError(true);
                    }}
                    onLoad={() => {
                      console.log('✅ EditProfileModal - Profile image loaded successfully');
                    }}
                  />
                );
              } else {
                console.log('🔍 EditProfileModal - No image URL, showing placeholder');
                return (
                  <View style={styles.profileImagePlaceholder}>
                    <FontAwesome name="user-md" size={40} color="#3498db" />
                  </View>
                );
              }
            })()}
