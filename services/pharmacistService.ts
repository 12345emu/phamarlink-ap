...
class PharmacistService {
  constructor() {
    console.log('🚨🚨🚨 PHARMACIST SERVICE CONSTRUCTOR CALLED - NEW VERSION 🚨🚨🚨');
  }
  async registerPharmacist(data: PharmacistRegistration): Promise<PharmacistRegistrationResponse> {
    console.log('🚨🚨🚨 PHARMACIST SERVICE FUNCTION CALLED - NEW VERSION 🚨🚨🚨');
    try {
      console.log('🚨🚨🚨 PHARMACIST SERVICE UPDATED - NEW VERSION RUNNING 🚨🚨🚨');
      ...
      if (data.profileImage) {
        ...
        // Try different FormData append methods for React Native
        try {
          formData.append('profileImage', imageFile as any);
          console.log('✅ Image appended to FormData successfully');
        } catch (error) {
          console.error('❌ Error appending image to FormData:', error);
          // Fallback: try without the file object wrapper
